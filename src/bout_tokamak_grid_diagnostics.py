...
# ==============================================================
# Chatwood Labs - BOUT++ 5.x Grid Diagnostics Utility (v2.0.0, BOUT++ 4.x / 5.x Compatible)
#
...
class Logging:
    ...
    class _SafeTextStream:
        """
        Text-stream proxy that never lets console encoding failures break logging.

        Windows terminals and redirected pipes may still expose a legacy encoding
        such as cp1252. Scientific output contains Unicode symbols (kappa, delta,
        approximately-equal, check marks), so a normal StreamHandler can otherwise
        raise UnicodeEncodeError while the diagnostics themselves are succeeding.

        When the underlying stream cannot represent a character, preserve the line
        using backslash escapes rather than raising from logging. GUI-launched child
        processes use UTF-8 explicitly, so this fallback is mainly for direct CLI use.
        """
    ...
    def setup_logging(*, verbose=False, quiet=False, log_file=None):
        ...
        sh_out = logging.StreamHandler(Logging._SafeTextStream(Logging._orig_stdout))
        ...
        sh_err = logging.StreamHandler(Logging._SafeTextStream(Logging._orig_stderr))
...
class GridIO:
    ...
    def detect_and_normalize_grid(ds):
        ...
        _maybe_alias("shiftAngle", "shiftangle")
...
def grid_scalar(ds, name, default=None):
    """
    Read a scalar grid quantity using the BOUT++-preferred representation.

    Current BOUT++ grid files store topology quantities such as ixseps1 and
    ixseps2 as scalar NetCDF variables. Historical/forensic files may instead
    carry them as global attributes, so attributes are retained as a fallback.
    """
    if name in ds.variables:
        try:
            return to_scalar(ds[name])
        except Exception:
            pass
...
def _close_periodic_y(*arrays):
    """Append the first poloidal column to close periodic plotting seams.

    BOUT++ grids correctly store y with endpoint=False, so y=2π is not duplicated.
    Matplotlib contour/surface routines do not infer that periodicity and otherwise
    leave a visible white wedge at the seam.  This helper is visualization-only.
    """
    closed=[]
    for arr in arrays:
        a=np.asarray(arr)
        if a.ndim < 2 or a.shape[1] < 1:
            ...
            closed.append(np.concatenate([a,a[:,0:1]],axis=1))
    return tuple(closed)

def _closed_curve(R,Z):
    R=np.asarray(R); Z=np.asarray(Z)
    if R.size==0: return R,Z
...
def render_grid_visualization(ds, output_path):
    ...
    try:
        ...
        for i in [0, nx//4, nx//2, 3*nx//4, nx-1]:
            ...
            R_plot,Z_plot=_closed_curve(R_slice,Z_slice)
            ax1.plot(R_plot, Z_plot, linewidth=1.5, alpha=0.7)
        ...
        if is_3d:
            ...
            # Close both periodic directions for plotting.  The stored grid quite
            # correctly omits duplicated 2π endpoints; plot_surface does not know
            # that and otherwise displays a seam.
            R_outer=np.concatenate([R_outer,R_outer[0:1,:]],axis=0)
            Z_outer=np.concatenate([Z_outer,Z_outer[0:1,:]],axis=0)
            R_outer=np.concatenate([R_outer,R_outer[:,0:1]],axis=1)
            Z_outer=np.concatenate([Z_outer,Z_outer[:,0:1]],axis=1)
            nphi = int(R_outer.shape[1]-1)
            phi = np.linspace(0, 2*np.pi, nphi+1, endpoint=True)
            ...
            R_curve,Z_curve = _closed_curve(R[-1, :], Z[-1, :])
            nphi = 64
            phi = np.linspace(0, 2*np.pi, nphi+1, endpoint=True)
        ...
        ax3.legend()
        # Suppress offset notation (e.g. "1e-14 + 2.686...") on primary axis —
        # it appears when R values are nearly constant and matplotlib applies
        # an automatic offset, making the label unreadable.
        ...
        # Also suppress on secondary axis
        ax3_twin.yaxis.get_major_formatter().set_useOffset(False)
        ax3_twin.yaxis.get_major_formatter().set_scientific(False)
        ...
        if has_curvature:
            ...
            R_grid = R
            Z_grid = Z
            R_grid_c,Z_grid_c,G1_c,G2_c = _close_periodic_y(R_grid,Z_grid,G1_slice,G2_slice)
            ...
            if norm1 is None:
                ...
                contour1 = ax4.contourf(R_grid_c, Z_grid_c, G1_c, levels=levels, cmap='RdBu_r', norm=norm1)
            ...
            for i in [0, nx//4, nx//2, 3*nx//4, nx-1]:
                rline,zline=_closed_curve(R[i,:],Z[i,:]); ax4.plot(rline,zline,'k-',linewidth=0.5,alpha=0.5)
            ...
            if norm2 is None:
                ...
                contour2 = ax5.contourf(R_grid_c, Z_grid_c, G2_c, levels=levels, cmap='RdBu_r', norm=norm2)
            ...
            for i in [0, nx//4, nx//2, 3*nx//4, nx-1]:
                rline,zline=_closed_curve(R[i,:],Z[i,:]); ax5.plot(rline,zline,'k-',linewidth=0.5,alpha=0.5)
            ...
            _,_,G_mag_c = _close_periodic_y(R_grid,Z_grid,G_mag)
            ...
            contour3 = ax6.contourf(R_grid_c, Z_grid_c, G_mag_c, levels=levels, cmap='viridis', norm=norm3)
            ...
            #R/Z are 2D (x,y): use R[i, :] not R[i, :, 0]
            for i in [0, nx//4, nx//2, 3*nx//4, nx-1]:
                rline,zline=_closed_curve(R[i,:],Z[i,:]); ax6.plot(rline,zline,'k-',linewidth=0.5,alpha=0.5)
...
def render_magnetic_field_visualization(ds, output_path):
    ...
    try:
        ...
        nx = R_slice.shape[0]
        ny = R_slice.shape[1]
        R_plot,Z_plot,Bxy_plot,Bpxy_plot = _close_periodic_y(
            R_slice,Z_slice,Bxy_slice,Bpxy_slice
        )
        ...
        if norm1 is None:
            ...
            contour1 = ax1.contourf(R_plot, Z_plot, Bxy_plot, levels=levels, cmap='plasma', norm=norm1)
        ...
        for i in flux_surface_idx:
            rline,zline=_closed_curve(R_slice[i,:],Z_slice[i,:]); ax1.plot(rline,zline,'k-',linewidth=0.8,alpha=0.4)
        ...
        if norm2 is None:
            ...
            contour2 = ax2.contourf(R_plot, Z_plot, Bpxy_plot, levels=levels, cmap='viridis', norm=norm2)
        ...
        for i in flux_surface_idx:
            rline,zline=_closed_curve(R_slice[i,:],Z_slice[i,:]); ax2.plot(rline,zline,'k-',linewidth=0.8,alpha=0.4)
        ...
        for i in flux_surface_idx:
            rline,zline=_closed_curve(R_slice[i,:],Z_slice[i,:]); ax3.plot(rline,zline,'k-',linewidth=1.2,alpha=0.6)
...
class Reports:
    ...
    def generate_html_report(path, ds, validation_results, output_html):
        ...
        try:
            ...
            geometry_model = validation_results.get('geometry_model', 'unknown')
            boundary_fidelity_rms = validation_results.get('boundary_fidelity_rms_m')
            boundary_fidelity_max = validation_results.get('boundary_fidelity_max_m')
            ...
            if geometry_model == 'custom_radial_boundary':
                geometry_model_label = 'CUSTOM BOUNDARY'
                elongation_label = 'Equivalent Elongation κ'
                aspect_label = 'Equivalent Aspect Ratio'
            elif geometry_model == 'miller':
                geometry_model_label = 'MILLER ANALYTIC'
                elongation_label = 'Elongation κ'
                aspect_label = 'Aspect Ratio'
            else:
                geometry_model_label = str(geometry_model).upper()
                elongation_label = 'Elongation κ'
                aspect_label = 'Aspect Ratio'

            if boundary_fidelity_max is not None:
                boundary_card = (
                    '<div class="info-card">'
                    '<h3>Boundary Fidelity</h3>'
                    '<div class="value" style="font-size: 1.1em;">'
                    f'RMS={boundary_fidelity_rms:.3e} m<br>'
                    f'max={boundary_fidelity_max:.3e} m'
                    '</div></div>'
                )
            else:
                boundary_card = ''
    ...
    <head>
        ...
        <style>
            * {{
            ...
            .verdict.warning {{
                background: linear-gradient(135deg, #f0ad4e 0%, #ffd36a 100%);
                color: #3b2a00;
            }}
    ...
    <body>
        <div class="container">
            ...
            <div class="verdict {'invalid' if len(critical_failures) else ('valid' if validation_results.get('bout5_compatible') else 'warning')}">
                <div class="icon">{'✗' if len(critical_failures) else ('✓' if validation_results.get('bout5_compatible') else '⚠')}</div>
                <div>{'Grid File INVALID' if len(critical_failures) else ('Grid File VALID - BOUT++ 5.x CANONICAL' if validation_results.get('bout5_compatible') else 'Numerically Valid - BOUT++ 5.x NON-CANONICAL')}</div>
            </div>
            ...
            <div class="content">
                <div class="section">
                    ...
                    <div class="grid-info">
                        ...
                        <div class="info-card">
                            <h3>Geometry Model</h3>
                            <div class="value" style="font-size: 1.2em;">{_h(geometry_model_label)}</div>
                        </div>
                        {boundary_card}
                        <div class="info-card">
                        ...
                        <div class="info-card">
                            <h3>{_h(elongation_label)}</h3>
                            <div class="value">{kappa:.2f}</div>
                        </div>
                        <div class="info-card">
                            <h3>{_h(aspect_label)}</h3>
                        ...
                        <div class="info-card">
                            <h3>BOUT++ 5.x Contract</h3>
                            <div class="value" style="font-size: 1.1em;">{"CANONICAL" if validation_results.get('bout5_compatible') else "NON-CANONICAL"}</div>
                        </div>
                        <div class="info-card">
...
class DiagnosticsRunner:
    ...
    def main(
        ...
        outer_x_idx = -1
        R_at_midplane = None
        geometry_model = str(ds.attrs.get("geometry_model", "unknown"))
        boundary_fidelity_rms = None
        boundary_fidelity_max = None
        ...
        if ("R" in ds.variables) and ("Z" in ds.variables):
            ...
            #
            # IMPORTANT: radial ordering is inferred from flux-surface *size*, not
            # <R>_theta. For triangular Miller surfaces the poloidal mean of R can
            # shift with radius even when x increases outward, so using mean(R) can
            # invert the inferred radial direction.
            R_avg = R.mean(dim="y", skipna=True)
            R_mean = float(to_scalar(R_avg.mean(skipna=True)))

            R_half_width = 0.5 * (
                R.max(dim="y", skipna=True) - R.min(dim="y", skipna=True)
            )
            radial_size_diff = R_half_width.diff(dim="x")
            radial_size_scale = max(
                abs(float(to_scalar(R_half_width.max(skipna=True)))),
                abs(float(to_scalar(R_half_width.min(skipna=True)))),
                1.0,
            )
            radial_size_tol = max(1e-10, 1e-8 * radial_size_scale)

            size_nondec = bool(to_scalar((radial_size_diff >= -radial_size_tol).all()))
            size_noninc = bool(to_scalar((radial_size_diff <= radial_size_tol).all()))
            size_has_inc = bool(to_scalar((radial_size_diff > radial_size_tol).any()))
            size_has_dec = bool(to_scalar((radial_size_diff < -radial_size_tol).any()))

            is_increasing = size_nondec and size_has_inc
            is_decreasing = size_noninc and size_has_dec

            if is_increasing:
                outer_x_idx = -1
                print("  [INFO] Radial coordinate: OUTWARD (flux-surface size increases with x)")
                check(True, "Flux-surface half-width increases with x (standard ordering)")
                record_check(
                    checks,
                    "geometry:radial_ordering",
                    True,
                    severity="INFO",
                    details={"ordering": "OUTWARD", "basis": "surface_half_width_R"},
                )
            elif is_decreasing:
                outer_x_idx = 0
                print("  [INFO] Radial coordinate: INWARD (flux-surface size decreases with x)")
                check(True, "Flux-surface half-width decreases with x (reversed ordering)")
                record_check(
                    checks,
                    "geometry:radial_ordering",
                    True,
                    severity="INFO",
                    details={"ordering": "INWARD", "basis": "surface_half_width_R"},
                )
            else:
                # Fall back to the widest surface for geometry extraction, but record
                # the ordering ambiguity as a soft diagnostic failure.
                outer_x_idx = int(to_scalar(R_half_width.argmax(dim="x")))
                print("  [WARN] Radial coordinate: NON-MONOTONIC surface size")
                check(False, "Flux-surface half-width monotonic ordering")
                record_check(
                    checks,
                    "geometry:radial_ordering",
                    False,
                    ...
                    details={
                        ...
                        "basis": "surface_half_width_R",
                        "message": "Flux-surface half-width is non-monotonic in x",
                    },
                )

            # Infer the outboard midplane y index once and retain the radial line for
            # the local ordering check below.
            theta_da = ds["theta"] if "theta" in ds else None
            y_mid_idx = infer_outboard_midplane_y_index(theta_da, R)
            R_at_midplane = R.isel(y=y_mid_idx)

            # Extract shape from the *outermost flux surface*. The previous code mixed
            # a radial line across all x with the global vertical extent, which could
            # report values such as kappa=3.78 for a known kappa=1.70 grid.
            R_outer_surf = R.isel(x=outer_x_idx)
            Z_outer_surf = Z.isel(x=outer_x_idx)

            R_out_max = float(to_scalar(R_outer_surf.max(skipna=True)))
            R_out_min = float(to_scalar(R_outer_surf.min(skipna=True)))
            Z_out_max = float(to_scalar(Z_outer_surf.max(skipna=True)))
            Z_out_min = float(to_scalar(Z_outer_surf.min(skipna=True)))

            R0_outer = 0.5 * (R_out_max + R_out_min)
            a_R = 0.5 * (R_out_max - R_out_min)
            a_Z = 0.5 * (Z_out_max - Z_out_min)

            if a_R > 1e-6:
                kappa = a_Z / a_R
                aspect_ratio = R0_outer / a_R

                top_idx = int(to_scalar(Z_outer_surf.argmax(dim="y")))
                bot_idx = int(to_scalar(Z_outer_surf.argmin(dim="y")))
                R_top = float(to_scalar(R_outer_surf.isel(y=top_idx)))
                R_bot = float(to_scalar(R_outer_surf.isel(y=bot_idx)))
                delta_approx = (R0_outer - 0.5 * (R_top + R_bot)) / a_R

                print(
                    f"  [INFO] Outer-surface geometry: R0={R0_outer:.3f}m, "
                    f"a={a_R:.3f}m, kappa={kappa:.3f}, delta~{delta_approx:.3f}"
                )
            else:
                kappa = 1.0
                delta_approx = 0.0
                print("  [INFO] Cylindrical geometry: a_R ~ 0, assuming kappa = 1.0")

            if abs(kappa - 1.0) < 0.02 and abs(delta_approx) < 0.02:
                print("  [INFO] Near-circular outer flux surface")
            else:
                print("  [INFO] Shaped outer flux surface detected")

            # If the generator preserved an explicit requested boundary, verify
            # that the actual outer grid surface matches it point-for-point at
            # the grid's y locations.  This is the WYSIWYG contract for custom
            # designer geometry.
            if "requested_boundary_R" in ds.variables and "requested_boundary_Z" in ds.variables:
                try:
                    req_R = ds["requested_boundary_R"]
                    req_Z = ds["requested_boundary_Z"]
                    if "y" in req_R.dims and int(req_R.sizes.get("y", 0)) == int(R_outer_surf.sizes.get("y", -1)):
                        dR = R_outer_surf - req_R
                        dZ = Z_outer_surf - req_Z
                        dist = np.sqrt(dR*dR + dZ*dZ)
                        boundary_fidelity_rms = float(to_scalar(np.sqrt((dist*dist).mean(skipna=True))))
                        boundary_fidelity_max = float(to_scalar(dist.max(skipna=True)))
                        boundary_scale = max(a_R, a_Z, 1.0)
                        fidelity_tol = max(1e-9, 1e-7 * boundary_scale)
                        fidelity_ok = bool(np.isfinite(boundary_fidelity_max) and boundary_fidelity_max <= fidelity_tol)
                        msg = (
                            f"Requested boundary fidelity: RMS={boundary_fidelity_rms:.3e} m, "
                            f"max={boundary_fidelity_max:.3e} m (tol={fidelity_tol:.3e} m)"
                        )
                        check(fidelity_ok, msg)
                        record_check(
                            checks, "geometry:requested_boundary_fidelity", fidelity_ok, severity="CRITICAL",
                            details={
                                "message": msg,
                                "rms_m": boundary_fidelity_rms,
                                "max_m": boundary_fidelity_max,
                                "tolerance_m": fidelity_tol,
                            },
                        )
                    else:
                        record_check(
                            checks, "geometry:requested_boundary_fidelity", False, severity="CRITICAL",
                            details={"message": "Requested boundary metadata has incompatible y dimensions"},
                        )
                except Exception as exc:
                    record_check(
                        checks, "geometry:requested_boundary_fidelity", False, severity="CRITICAL",
                        details={"message": f"Could not evaluate requested boundary fidelity: {exc}"},
                    )

            # R at the inferred outboard midplane should be monotonic with x,
            # consistent with the surface-size ordering above. A small tolerance
            # prevents floating-point noise or repeated guard values from false-failing.
            R_outboard_line = R_at_midplane
            outboard_diff = R_outboard_line.diff(dim="x")
            outboard_scale = max(
                abs(float(to_scalar(R_outboard_line.max(skipna=True)))),
                abs(float(to_scalar(R_outboard_line.min(skipna=True)))),
                1.0,
            )
            outboard_tol = max(1e-10, 1e-8 * outboard_scale)

            if is_increasing:
                ok = bool(to_scalar((outboard_diff >= -outboard_tol).all()))
                msg = "R is non-decreasing with x at outboard midplane (standard ordering)"
            elif is_decreasing:
                ok = bool(to_scalar((outboard_diff <= outboard_tol).all()))
                msg = "R is non-increasing with x at outboard midplane (reversed ordering)"
            else:
                ok = False
            ...
            record_check(
                checks,
                "geometry:R_outboard_monotonic_x",
                bool(ok),
                severity="WARN",
                details={"message": msg},
            )
            ...
            if geometry_model == "custom_radial_boundary":
                # Arbitrary custom boundaries need not be vertically symmetric about
                # global Z=0 (or even about their magnetic axis).  The old Z-mean
                # heuristic therefore produced false warnings for legitimate shifted
                # or asymmetric custom shapes.  What is structurally required is that
                # the recorded magnetic axis lies within the generated vertical span.
                try:
                    axis_Z = float(ds.attrs.get("axis_Z"))
                except (TypeError, ValueError):
                    axis_Z = None

                Z_min = float(to_scalar(Z.min()))
                Z_max = float(to_scalar(Z.max()))
                Z_scale = max(abs(Z_min), abs(Z_max), 1.0)
                tol_Z = max(1e-8, 1e-8 * Z_scale)

                if axis_Z is None or not np.isfinite(axis_Z):
                    _axis_ok = False
                    _axis_msg = "Custom boundary magnetic-axis Z metadata is present and finite"
                    _axis_details = {
                        "message": _axis_msg,
                        "axis_Z": axis_Z,
                        "Z_min": Z_min,
                    ...
                    _axis_ok = (Z_min - tol_Z) <= axis_Z <= (Z_max + tol_Z)
                    _axis_msg = (
                        "Custom-boundary magnetic axis lies within generated vertical extent"
                    )
                    _axis_details = {
                        "message": _axis_msg,
                        "axis_Z": float(axis_Z),
                ...
                check(_axis_ok, _axis_msg)
                record_check(
                    checks,
                    "geometry:custom_axis_within_vertical_extent",
                    bool(_axis_ok),
                    severity="CRITICAL",
                    details=_axis_details,
                )
            else:
                # Soft symmetry sanity check for analytic Miller-style grids, whose
                # canonical construction is referenced to the global midplane Z=0.
                ...
                record_check(
                    checks,
                    "geometry:Z_mean_near_zero",
                    bool(_z_ok),
                    severity="WARN",
                    details={
                        "tol": float(tol_Z),
                        "Z_mean": float(to_scalar(Z.mean())),
                        "message": _z_msg,
                    },
                )

            # The toroidal R^2 metric check is convention-sensitive.  In canonical
            # BOUT++ files g33 is contravariant while g_33 is covariant, so testing
            # g33 against R^2 would be wrong for precisely the files we most want to
            # validate.  Defer this check until the determinant section has identified
            # whether the un-underscored tensor is mathematically covariant or
            # contravariant.
        ...
        #
        # BOUT++ stores ixseps1/ixseps2 as scalar grid variables. The explicit
        # convention ixseps1=ixseps2=nx means there is no separatrix inside the
        # represented radial domain: all points are in the closed-core region.
        ixseps1 = grid_scalar(ds, 'ixseps1', -1)
        ixseps2 = grid_scalar(ds, 'ixseps2', -1)
        jyseps1_1 = grid_scalar(ds, 'jyseps1_1', -1)
        jyseps2_1 = grid_scalar(ds, 'jyseps2_1', -1)

        closed_core_no_xpoint = (
            ixseps1 is not None
            and ixseps2 is not None
            and int(ixseps1) >= nx
            and int(ixseps2) >= nx
        )

        if closed_core_no_xpoint:
            config_type = "CLOSED_CORE / NO X-POINT"
            print("  [INFO] Configuration: CLOSED_CORE / NO X-POINT")
            print("  [INFO] ixseps1=ixseps2=nx: no separatrix lies inside the generated domain")
        elif ixseps1 > 0 and (jyseps1_1 > 0 or jyseps2_1 > 0):
            ...
            config_type = "LIMITER"
            print("  [INFO] Configuration: LIMITER / OPEN-EDGE (no X-point)")
            print("  [INFO] No explicit closed-core topology declaration detected")
        
        # Grid quality assessment
        print("  [INFO] Grid Quality Assessment:")
        
        # Aspect ratio and shaping were already extracted from the outermost
        # surface selected using the detected radial ordering. Reuse those values
        # here instead of hard-coding x=-1 (which is wrong for inward-ordered grids).
        if R0_outer is not None and a_R > 1e-6:
            aspect_ratio = R0_outer / a_R
            surface_label = (
                "Equivalent outer-surface descriptors"
                if geometry_model == "custom_radial_boundary"
                else "Outer surface"
            )
            print(
                f"    {surface_label}: R0={R0_outer:.3f}m  a={a_R:.3f}m  "
                f"κ={kappa:.3f}  δ≈{delta_approx:.3f}  A={aspect_ratio:.3f}"
            )
        elif R_mean > 0 and a_R > 0:
            aspect_ratio = R_mean / a_R

        if aspect_ratio > 0:
            aspect_prefix = "Equivalent " if geometry_model == "custom_radial_boundary" else ""
            print(f"    {aspect_prefix}aspect ratio A = R0/a = {aspect_ratio:.2f}")
            if aspect_ratio > 10:
        ...
        # Shaping parameter summary from the same outer surface used above.
        if (R is not None) and (Z is not None) and a_R > 1e-6:
            descriptor_prefix = "Equivalent " if geometry_model == "custom_radial_boundary" else ""
            print(
                f"    {descriptor_prefix}elongation κ = {kappa:.2f} "
                f"{'(moderately elongated)' if kappa < 1.5 else '(highly elongated)' if kappa > 1.5 else ''}"
            )
            if abs(delta_approx) > 0.01 and abs(delta_approx) < 2.0:
                print(
                    f"    {descriptor_prefix}triangularity δ ≈ {delta_approx:.2f} "
                    "(outer-surface geometric estimate)"
                )
        ...
        if "zShift" in ds:
            shift_name = "zShift"
        elif "shiftAngle" in ds:
            # Historical/non-canonical 2D shift field retained for forensic reads.
        ...
        #
        # Forensic convention detection:
        # Some historical/non-BOUT toolchains use g11..g23 as covariant even though
        # BOUT++ 5.x defines these names as CONTRAVARIANT g^{ij}.  Keep autodetection
        # because it is useful when inspecting arbitrary files, but report BOUT++
        # compatibility separately rather than treating both conventions as equivalent.
        #
        # We first identify the mathematical convention by comparing:
        ...
        # ----------------------------------------------------------
        # Canonical BOUT++ 5.x metric contract
        # ----------------------------------------------------------
        # BOUT++ has fixed semantics:
        #   g11..g23    = contravariant g^{ij}
        #   g_11..g_23  = covariant g_ij
        # and its loader checks J and Bxy against those tensors.  A file can be
        # mathematically self-consistent under the reversed naming convention
        # yet still be unsuitable for BOUT++ differential operators.
        source_contract = str(ds.attrs.get("grid_contract", "UNKNOWN"))
        source_claims_bout5 = bool(int(ds.attrs.get("bout5_canonical", 0))) if str(ds.attrs.get("bout5_canonical", "0")).strip() not in ("", "None") else False

        bout5_metric_naming = "CANONICAL" if mean_con <= mean_cov else "REVERSED_NONCANONICAL"
        bout5_j_max_rel_error = None
        bout5_bxy_max_rel_error = None
        bout5_shift_torsion_present = False
        bout5_shift_torsion_finite = False
        bout5_compatible = False
        bout5_reason = None

        required_cov_names = ("g_11", "g_22", "g_33", "g_12", "g_13", "g_23")
        have_covariant_set = all(name in ds for name in required_cov_names)

        if have_covariant_set:
            cg11, cg22, cg33 = ds["g_11"], ds["g_22"], ds["g_33"]
            cg12, cg13, cg23 = ds["g_12"], ds["g_13"], ds["g_23"]
            det_cov_bout = (
                cg11 * (cg22 * cg33 - cg23**2)
                - cg12 * (cg12 * cg33 - cg13 * cg23)
                + cg13 * (cg12 * cg23 - cg13 * cg22)
            )
            J_from_bout_metric = xr.apply_ufunc(np.sqrt, xr.apply_ufunc(np.abs, det_cov_bout, dask="allowed"), dask="allowed")
            j_rel_bout = xr.apply_ufunc(np.abs, J_from_bout_metric - J_local, dask="allowed") / (xr.apply_ufunc(np.abs, J_local, dask="allowed") + 1e-30)
            bout5_j_max_rel_error = float(to_scalar(j_rel_bout.max(skipna=True)))

            if "Bxy" in ds:
                B_from_bout_metric = xr.apply_ufunc(np.sqrt, xr.apply_ufunc(np.abs, cg22, dask="allowed"), dask="allowed") / (xr.apply_ufunc(np.abs, J_local, dask="allowed") + 1e-30)
                b_rel_bout = xr.apply_ufunc(np.abs, B_from_bout_metric - ds["Bxy"], dask="allowed") / (xr.apply_ufunc(np.abs, ds["Bxy"], dask="allowed") + 1e-30)
                bout5_bxy_max_rel_error = float(to_scalar(b_rel_bout.max(skipna=True)))

            # BOUT++ reads ShiftTorsion directly and warns that vector derivatives
            # may be incorrect when it is absent.  Treat it as part of the claimed
            # canonical Grid Suite contract while retaining WARN-level forensic
            # reporting for third-party/historical grids.
            bout5_shift_torsion_present = "ShiftTorsion" in ds
            bout5_shift_torsion_finite = False
            if bout5_shift_torsion_present:
                bout5_shift_torsion_finite = bool(
                    to_scalar(
                        xr.apply_ufunc(np.isfinite, ds["ShiftTorsion"], dask="allowed").all()
                    )
                )

            canonical_ok = (
                bout5_metric_naming == "CANONICAL"
                and bout5_j_max_rel_error is not None and bout5_j_max_rel_error < 1e-8
                and bout5_bxy_max_rel_error is not None and bout5_bxy_max_rel_error < 1e-8
                and bout5_shift_torsion_present
                and bout5_shift_torsion_finite
            )
            bout5_compatible = bool(canonical_ok)

            severity = "CRITICAL" if source_claims_bout5 else "WARN"
            record_check(
                checks, "bout5:metric_naming_canonical",
                bout5_metric_naming == "CANONICAL", severity=severity,
                details={
                    "interpretation": bout5_metric_naming,
                    "required": "g11..g23=contravariant; g_11..g_23=covariant",
                },
            )
            record_check(
                checks, "bout5:jacobian_from_covariant_metric",
                bout5_j_max_rel_error < 1e-8, severity=severity,
                details={"max_relative_error": bout5_j_max_rel_error},
            )
            record_check(
                checks, "bout5:Bxy_from_metric",
                bout5_bxy_max_rel_error < 1e-8, severity=severity,
                details={"max_relative_error": bout5_bxy_max_rel_error},
            )
            record_check(
                checks, "bout5:ShiftTorsion_present_and_finite",
                bout5_shift_torsion_present and bout5_shift_torsion_finite,
                severity=severity,
                details={
                    "present": bool(bout5_shift_torsion_present),
                    "finite": bool(bout5_shift_torsion_finite),
                    "reason": "BOUT++ uses ShiftTorsion=d(nu)/dx for vector differential operators",
                },
            )

            if bout5_compatible:
                print(
                    "  [OK] BOUT++ 5.x canonical contract: metric names, J and Bxy agree "
                    f"(max rel dJ={bout5_j_max_rel_error:.3e}, dB={bout5_bxy_max_rel_error:.3e})"
                )
            else:
                bout5_reason = (
                    "BOUT++ 5.x interprets g11..g23 as contravariant and g_11..g_23 as covariant. "
                    "This file does not satisfy that loader contract. BOUT++ may still open the file and "
                    "accept supplied J/Bxy, but its differential operators will use the metric tensors under "
                    "the canonical naming semantics, so the grid must not be treated as BOUT++ 5.x valid."
                )
                print(f"  [WARN] BOUT++ 5.x NON-CANONICAL: {bout5_reason}")
        else:
            bout5_reason = "Full covariant metric set g_11..g_23 is not present; canonical BOUT++ loader cross-check is incomplete."
            print(f"  [INFO] {bout5_reason}")

        # ----------------------------------------------------------
        # Convention-aware toroidal metric geometry check
        # ----------------------------------------------------------
        # For the field-aligned coordinates documented by BOUT++, the covariant
        # toroidal component is g_zz = R^2.  Which NetCDF variable contains the
        # covariant tensor depends on the convention detected above:
        #   canonical BOUT++:       g_33
        #   historical/reversed:    g33
        # This preserves forensic support without falsely rejecting canonical files.
        if "R" in ds.variables:
            ...
            toroidal_cov_name = (
                "g_33" if metric_interpretation == "CONTRAVARIANT" else "g33"
            )
            convention_note = (
                "canonical BOUT++ covariant tensor"
                if toroidal_cov_name == "g_33"
                else "forensic reversed convention (un-underscored tensor is covariant)"
            )

            if toroidal_cov_name in ds.variables:
                toroidal_cov = ds[toroidal_cov_name]
                if "z" in toroidal_cov.dims:
                    toroidal_cov = toroidal_cov.isel(z=0)

                R_for_metric = ds["R"]
                if "z" in R_for_metric.dims:
                    R_for_metric = R_for_metric.isel(z=0)

                do_R2_check = any(u in z_units for u in ("rad", "radian", "radians"))
                ratio_da = toroidal_cov / (R_for_metric**2)

                if do_R2_check:
                    R_scale = float(to_scalar(R_for_metric.mean()))
                    tol_R2 = max(1e-6, 1e-6 * R_scale**2)
                    close_da = xr.apply_ufunc(
                        np.isclose,
                        toroidal_cov,
                        R_for_metric**2,
                        kwargs={"rtol": 0.0, "atol": tol_R2},
                        dask="allowed",
                    )
                    R2_ok = bool(to_scalar(close_da.all()))
                    R2_msg = (
                        f"{toroidal_cov_name} ~= R^2 "
                        f"(covariant toroidal metric, tol={tol_R2:.2e}, z units='{z_units or 'unknown'}')"
                    )
                    check(R2_ok, R2_msg)
                    record_check(
                        checks,
                        "geometry:covariant_toroidal_metric_matches_R2",
                        R2_ok,
                        severity=("CRITICAL" if source_claims_bout5 else "WARN"),
                        details={
                            "metric_variable": toroidal_cov_name,
                    ...
                    record_check(
                        ...
                        severity="INFO",
                        details={
                            "reason": "z coordinate is not labelled as radians",
                            "metric_variable": toroidal_cov_name,
                            "metric_interpretation": metric_interpretation,
                            "convention": convention_note,
                            "z_units": (z_units or "unknown"),
                ...
                record_check(
                    ...
                    severity="INFO",
                    details={
                        "reason": f"Detected covariant metric variable {toroidal_cov_name} is absent",
                        "metric_interpretation": metric_interpretation,
                    },
                )
        ...
        ixseps1 = grid_scalar(ds, 'ixseps1', None)
        ixseps2 = grid_scalar(ds, 'ixseps2', None)
        ...
        # ----------------------------------------------------------
        # Canonical ShiftAngle validation
        # ----------------------------------------------------------
        print("\n=== ShiftAngle / twist-shift ===")
        if "ShiftAngle" in ds:
            sa_da = ds["ShiftAngle"]
            sa_min = float(to_scalar(sa_da.min(skipna=True)))
            sa_max = float(to_scalar(sa_da.max(skipna=True)))
            print(f"  ShiftAngle(x): min={sa_min:.4f} max={sa_max:.4f} rad")
            _sa_finite = bool(to_scalar(xr.apply_ufunc(np.isfinite, sa_da, dask="allowed").all()))
            check(_sa_finite, "ShiftAngle finite")
            record_check(checks, "shift:ShiftAngle:finite", _sa_finite, severity="CRITICAL")

            if "q" in ds and tuple(ds["q"].dims) == tuple(sa_da.dims):
                q_from_shift = sa_da / (2.0 * np.pi)
                q_rel = xr.apply_ufunc(np.abs, q_from_shift - ds["q"], dask="allowed") / (xr.apply_ufunc(np.abs, ds["q"], dask="allowed") + 1e-30)
                q_shift_max_rel = float(to_scalar(q_rel.max(skipna=True)))
                _q_shift_ok = q_shift_max_rel < 1e-8
                check(_q_shift_ok, "ShiftAngle/(2π) matches q(x)")
                record_check(
                    checks, "shift:ShiftAngle:q_consistency", _q_shift_ok, severity="CRITICAL",
                    details={"max_relative_error": q_shift_max_rel},
                )
        elif "shiftAngle" in ds:
            print("  [INFO] Historical 2D shiftAngle field detected; canonical BOUT++ uses zShift(x,y) plus ShiftAngle(x).")
        ...
        # Keep the top-level critical list consistent with structured checks.
        # This is especially important for custom-boundary fidelity checks, which
        # are recorded through the shared check framework rather than bespoke
        # verdict code.
        for entry in checks:
            if str(entry.get("severity", "")).upper() != "CRITICAL":
                continue
            if str(entry.get("status", "")).upper() != "FAIL":
                continue
            details = entry.get("details")
            if isinstance(details, dict):
                failure_text = details.get("message")
            else:
                failure_text = str(details) if details else None
            if not failure_text:
                failure_text = f"{entry.get('name', 'CRITICAL check')} failed"
            if failure_text not in critical_failures:
                critical_failures.append(failure_text)

        # Keep the top-level warning list consistent with the structured check
        # records used by the HTML/JSON report. Any failed WARN check is, by
        # definition, a warning and must appear in the final verdict as well.
        for entry in checks:
            if str(entry.get("severity", "")).upper() != "WARN":
                continue
            if str(entry.get("status", "")).upper() != "FAIL":
                continue
            ...
            details = entry.get("details")
            if isinstance(details, dict):
                warning_text = details.get("message")
            else:
                warning_text = str(details) if details else None

            if not warning_text:
                warning_text = f"{entry.get('name', 'WARN check')} failed"
        ...
        if len(critical_failures) == 0:
            if 'bout5_compatible' in locals() and bout5_compatible:
                print("Status: ✓ GRID FILE VALID - BOUT++ 5.x CANONICAL")
                print("\nThis grid file passes all critical validation checks and")
                print("satisfies the canonical BOUT++ 5.x metric/J/Bxy contract.")
            elif 'bout5_compatible' in locals() and bout5_compatible is False:
                print("Status: ⚠ GRID NUMERICALLY CONSISTENT - BOUT++ 5.x NON-CANONICAL")
                print("\nThis file passes the generic numerical checks, but does NOT satisfy")
                print("the canonical BOUT++ 5.x metric/J/Bxy naming contract.")
                print("BOUT++ may open the file and accept supplied J/Bxy, while still")
                print("interpreting g11..g23 as contravariant and g_11..g_23 as covariant.")
                print("Do not treat this grid as valid for BOUT++ 5.x simulation use.")
            else:
                print("Status: ✓ GRID FILE NUMERICALLY VALID")
                print("\nThis grid file passes the available critical numerical checks.")
            
            print(f"\nGrid Type: {grid_type}")
            print(f"Configuration: {config_type}")
            print(f"Dimensions: nx={nx}, ny={ny}, nz={nz}")
            if geometry_model == "custom_radial_boundary":
                print(f"Equivalent Elongation: κ={kappa:.2f}")
                print(f"Equivalent Aspect Ratio: A={aspect_ratio:.2f}")
            else:
            ...
            if len(warnings) > 0:
                ...
                for w in warnings:
                    print(f"  • {w}")
                print("\nWarnings indicate potential issues; BOUT++ compatibility must be assessed separately.")
            else:
        ...
        validation_results = {
            ...
            'boundary_fidelity_rms_m': boundary_fidelity_rms,
            'boundary_fidelity_max_m': boundary_fidelity_max,
            'R0': R0_outer,
            'a': a_R,
            'kappa': kappa,
            'delta': delta_approx,
            ...
            'bout5_compatible': (bout5_compatible if 'bout5_compatible' in locals() else None),
            'bout5_metric_naming': (bout5_metric_naming if 'bout5_metric_naming' in locals() else None),
            'bout5_j_max_rel_error': (bout5_j_max_rel_error if 'bout5_j_max_rel_error' in locals() else None),
            'bout5_bxy_max_rel_error': (bout5_bxy_max_rel_error if 'bout5_bxy_max_rel_error' in locals() else None),
            'bout5_shift_torsion_present': (bout5_shift_torsion_present if 'bout5_shift_torsion_present' in locals() else None),
            'bout5_shift_torsion_finite': (bout5_shift_torsion_finite if 'bout5_shift_torsion_finite' in locals() else None),
            'bout5_reason': (bout5_reason if 'bout5_reason' in locals() else None),
            'grid_contract': (source_contract if 'source_contract' in locals() else str(ds.attrs.get('grid_contract', 'UNKNOWN'))),
...
def main_cli(argv=None) -> int:
    ...
    # Parse argv if provided; otherwise argparse uses sys.argv as usual.
